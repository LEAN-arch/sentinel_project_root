...
try:
...
except ImportError as e_import_clinic_dash:
    error_msg_clinic_dash = (
        ...
        "Also, ensure all external libraries (like pandas, plotly, geojson) are installed in your environment (see requirements.txt)."
    )
...
# Page Configuration
APP_NAME = getattr(app_config, 'APP_NAME', 'Sentinel Health Co-Pilot')
APP_LOGO_SMALL = getattr(app_config, 'APP_LOGO_SMALL', 'logo.png')
st.set_page_config(
    page_title=f"Clinic Console - {APP_NAME}",
    layout="wide",
...
@st.cache_data(
    ttl=getattr(app_config, 'CACHE_TTL_SECONDS_WEB_REPORTS', 3600),
    show_spinner="Loading comprehensive clinic operational dataset...",
    hash_funcs={pd.DataFrame: lambda df: hash(df.to_string())}
)
def get_clinic_console_data(
    ...
    logger.debug(f"({func_log_prefix}) Loading data for period: {selected_start_date.isoformat()} to {selected_end_date.isoformat()}")
    ...
    if not full_health_enriched.empty and 'encounter_date' in full_health_enriched.columns:
        ...
        full_health_enriched.set_index('encounter_date', inplace=True)
        df_period_health_clinic = full_health_enriched[
            (full_health_enriched.index.date >= selected_start_date) &
            (full_health_enriched.index.date <= selected_end_date)
        ].reset_index()
    ...
    if not df_period_health_clinic.empty:
        ...
        logger.debug(f"({func_log_prefix}) No health data in selected period for clinic summary KPIs.")
        dict_period_summary_clinic = {"test_summary_details": {}}
        
    return full_health_enriched.reset_index(), df_period_health_clinic, df_period_iot_clinic, dict_period_summary_clinic, iot_data_source_available

# Page Title & Sidebar Filters
st.title(f"🏥 {APP_NAME} - Clinic Operations & Management Console")
st.markdown("**Service Performance, Patient Care Quality, Resource Management, and Facility Environment Monitoring**")
st.divider()

if os.path.exists(APP_LOGO_SMALL):
    st.sidebar.image(APP_LOGO_SMALL, width=150)
st.sidebar.header("🗓️ Console Filters")

default_days_console = getattr(app_config, 'WEB_DASHBOARD_DEFAULT_DATE_RANGE_DAYS_TREND', 30)
...
if default_start_dt_console_val < min_date_console_picker:
    default_start_dt_console_val = min_date_console_picker
...
if (selected_end_date_clinic_console - selected_start_date_clinic_console).days > 90:
    st.sidebar.error("Date range cannot exceed 90 days.")
    st.stop()
...
full_historical_health_df, period_health_data_clinic_tabs, period_iot_data_clinic_tabs, period_clinic_summary_kpis_data, iot_source_is_available_flag = get_clinic_console_data(selected_start_date_clinic_console, selected_end_date_clinic_console)

if not iot_source_is_available_flag:
    st.sidebar.warning("IoT data source is disabled. Check configuration.")
...
if period_clinic_summary_kpis_data and isinstance(period_clinic_summary_kpis_data.get("test_summary_details"), dict):
    ...
    if main_kpi_list_clinic:
        ...
        for i, kpi_data_main in enumerate(main_kpi_list_clinic):
            with cols_main_kpi_cards_clinic[i % 4]:
                render_web_kpi_card(**kpi_data_main)
    if disease_kpi_list_clinic:
        ...
        for i, kpi_data_disease in enumerate(disease_kpi_list_clinic):
            with cols_disease_kpi_cards_clinic[i % 4]:
                render_web_kpi_card(**kpi_data_disease)
else:
...
if has_meaningful_env_data_clinic:
    ...
    with cols_env_kpi_cards_clinic[0]:
        co2_value = env_kpi_dict_clinic.get('avg_co2_ppm_overall', np.nan)
        render_web_kpi_card(
            "Avg. CO2", f"{co2_value:.0f}" if pd.notna(co2_value) else "N/A",
            units="ppm", icon="💨", status_level=env_kpi_dict_clinic.get('co2_status_level', "NEUTRAL"),
            help_text=f"Target < {getattr(app_config, 'ALERT_AMBIENT_CO2_HIGH_PPM', 800)}ppm"
        )
    with cols_env_kpi_cards_clinic[1]:
        pm25_value = env_kpi_dict_clinic.get('avg_pm25_ugm3_overall', np.nan)
        render_web_kpi_card(
            "Avg. PM2.5", f"{pm25_value:.1f}" if pd.notna(pm25_value) else "N/A",
            units="µg/m³", icon="🌫️", status_level=env_kpi_dict_clinic.get('pm25_status_level', "NEUTRAL"),
            help_text=f"Target < {getattr(app_config, 'ALERT_AMBIENT_PM25_HIGH_UGM3', 35)}µg/m³"
        )
    with cols_env_kpi_cards_clinic[2]:
        occupancy_value = env_kpi_dict_clinic.get('avg_waiting_room_occupancy_persons', np.nan)
        render_web_kpi_card(
            "Avg. Waiting Occupancy", f"{occupancy_value:.1f}" if pd.notna(occupancy_value) else "N/A",
            units="persons", icon="👨‍👩‍👧‍👦", status_level=env_kpi_dict_clinic.get('occupancy_status_level', "NEUTRAL"),
            help_text=f"Target < {getattr(app_config, 'TARGET_CLINIC_WAITING_ROOM_OCCUPANCY_MAX', 20)} persons"
        )
    with cols_env_kpi_cards_clinic[3]:
        noise_alert_count_clinic = env_kpi_dict_clinic.get('noise_rooms_at_high_alert_count', 0)
        render_web_kpi_card(
            "High Noise Alerts", str(noise_alert_count_clinic),
            units="rooms", icon="🔊", status_level="HIGH_CONCERN" if noise_alert_count_clinic > 0 else "ACCEPTABLE",
            help_text=f"Rooms with noise > {getattr(app_config, 'ALERT_AMBIENT_NOISE_HIGH_DBA', 60)}dBA"
        )
else:
    if iot_source_is_available_flag:
        st.info("No significant environmental IoT data for this period to display snapshot KPIs.")
    else:
        st.caption("Environmental IoT data source is generally unavailable for this clinic. Monitoring limited.")
st.divider()
    ...
    if not period_health_data_clinic_tabs.empty:
        ...
        if isinstance(df_symptom_trends_display, pd.DataFrame) and not df_symptom_trends_display.empty:
            st.plotly_chart(plot_bar_chart_web(
                df_symptom_trends_display, x_col='week_start_date', y_col='count', color_col='symptom',
                title="Weekly Symptom Frequency (Top Reported)", barmode='group', y_is_count=True,
                x_axis_label="Week Starting", y_axis_label="Symptom Encounters"
            ), use_container_width=True)
        
        malaria_rdt_config_name = getattr(app_config, 'KEY_TEST_TYPES_FOR_ANALYSIS', {}).get("RDT-Malaria", {}).get("display_name", "Malaria RDT")
        ...
        if isinstance(series_malaria_positivity_trend, pd.Series) and not series_malaria_positivity_trend.empty:
            st.plotly_chart(plot_annotated_line_chart_web(
                series_malaria_positivity_trend, chart_title=f"Weekly {malaria_rdt_config_name} Positivity Rate",
                y_axis_label="Positivity %", target_ref_line=getattr(app_config, 'TARGET_MALARIA_POSITIVITY_RATE', 10),
                y_is_count=False
            ), use_container_width=True)
        
        if epi_data_for_display_tab.get("calculation_notes"):
            for note_epi in epi_data_for_display_tab["calculation_notes"]:
                st.caption(f"Epi Note: {note_epi}")
    else:
...
with tab_clinic_testing_view:
    ...
    if dict_testing_insights_data.get("processing_notes"):
        for note_test in dict_testing_insights_data["processing_notes"]:
            st.caption(f"Testing Note: {note_test}")
...
with tab_clinic_supply_view:
    ...
    if list_forecast_items_summary:
        st.dataframe(
            pd.DataFrame(list_forecast_items_summary), use_container_width=True, hide_index=True,
    ...
    if dict_supply_forecast_data.get("data_processing_notes"):
        for note_supply in dict_supply_forecast_data["data_processing_notes"]:
            st.caption(f"Supply Note: {note_supply}")
...
with tab_clinic_patient_view:
    ...
    if not period_health_data_clinic_tabs.empty:
        ...
        if dict_patient_focus_data.get("processing_notes"):
            for note_patient in dict_patient_focus_data["processing_notes"]:
                st.caption(f"Patient Focus Note: {note_patient}")
    else:
        st.info("No health data in the selected period for patient focus analysis.")
...
with tab_clinic_env_view:
    ...
    if isinstance(series_co2_trend_clinic_display, pd.Series) and not series_co2_trend_clinic_display.empty:
        st.plotly_chart(plot_annotated_line_chart_web(
            series_co2_trend_clinic_display, "Hourly Avg. CO2 Levels (Clinic-wide)",
            y_axis_label="CO2 (ppm)", date_format="%H:%M (%d-%b)", y_is_count=False
        ), use_container_width=True)
    ...
    if dict_env_details_display.get("processing_notes"):
        for note_env in dict_env_details_display["processing_notes"]:
            st.caption(f"Env. Detail Note: {note_env}")
...
logger.debug(f"Clinic Operations & Management Console page loaded/refreshed for period: {current_period_str_for_display}")
```

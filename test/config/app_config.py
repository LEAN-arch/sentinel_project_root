...
APP_LOGO_SMALL = os.path.join(ASSETS_DIR, "sentinel_logo_small.png") # Assume these exist or handle missing in UI
...
HEAT_STRESS_BODY_TEMP_TARGET_C = 37.5 # Target for cooling efforts
HEAT_STRESS_RISK_BODY_TEMP_C = 38.5   # Skin/body temp indicating heat stress risk
...
STRESS_HRV_LOW_THRESHOLD_MS = 20      # Example RMSSD/SDNN for HRV stress indication
...
RISK_SCORE_HIGH_THRESHOLD = 75     # Red (for patient AI risk score)

DISTRICT_ZONE_HIGH_RISK_AVG_SCORE = 70 # DHO level: avg zone AI risk score > this = high risk zone
DISTRICT_INTERVENTION_FACILITY_COVERAGE_LOW_PCT = 60 # DHO level target
DISTRICT_INTERVENTION_TB_BURDEN_HIGH_ABS = 10 # Absolute TB cases per zone for intervention flag
...
TARGET_DAILY_STEPS = 8000 # General wellness target reference
...
# --- IV. Data Input Configuration ---
# Schemas could be used for validation if system grows, e.g.:
...
FACILITY_NODE_DB_TYPE = "POSTGRESQL" # Example for a larger facility
FHIR_SERVER_ENDPOINT_LOCAL = "http://localhost:8080/fhir" # Example local FHIR server
DEFAULT_REPORT_INTERVAL_HOURS = 24
CACHE_TTL_SECONDS_WEB_REPORTS = 3600 # 1 hour for Streamlit web report data
...
KEY_TEST_TYPES_FOR_ANALYSIS = {
    ...
    # Add other relevant tests if used in analysis
}
...
OVERDUE_PENDING_TEST_DAYS_GENERAL_FALLBACK = 7 # If specific TAT not found for a test
...
TARGET_MALARIA_POSITIVITY_RATE = 5.0 # Target % for malaria test positivity (example)

# --- VII. Web Dashboarding Configuration (Tiers 2/3) ---
TIJUANA_CENTER_LAT = 32.5149 # Retained as an example, can be made generic
TIJUANA_CENTER_LON = -117.0382
TIJUANA_DEFAULT_ZOOM = 10

WEB_DASHBOARD_DEFAULT_DATE_RANGE_DAYS_VIEW = 1   # For daily snapshots
WEB_DASHBOARD_DEFAULT_DATE_RANGE_DAYS_TREND = 30 # For trend views
...
MAPBOX_STYLE_WEB = "carto-positron" # Default open style; actual choice managed by ui_helpers based on token availability
DEFAULT_CRS_STANDARD = "EPSG:4326" # Standard WGS84 for GeoJSON and general mapping

LOG_LEVEL = os.getenv("SENTINEL_LOG_LEVEL", "INFO") # Allow override by ENV VAR
LOG_FORMAT = '%(asctime)s - %(name)s - %(levelname)s - [%(module)s.%(funcName)s:%(lineno)d] - %(message)s'
LOG_DATE_FORMAT = '%Y-%m-%d %H:%M:%S'

# --- VIII. Sentinel System Color Palette (for consistency in UIs) ---
# These are primary definitions. CSS variables in style_web_reports.css should mirror these.
...
COLOR_ACCENT_BRIGHT = "#4D7BF3" # For specific accents like h1 underline
...
COLOR_TEXT_LINK_DEFAULT = COLOR_ACTION_PRIMARY # Use primary action color for links
...
# Legacy disease colors for web reports (if still needed for specific charts)
# It's better to configure disease-specific colors more robustly if widely used.
...
# Auto-populate from KEY_CONDITIONS_FOR_ACTION if not explicitly defined.
for cond in KEY_CONDITIONS_FOR_ACTION:
    if cond not in LEGACY_DISEASE_COLORS_WEB:
        # Basic heuristic for assigning related colors, can be refined
        ...
        else: LEGACY_DISEASE_COLORS_WEB[cond] = "#A9A9A9" # Default grey
